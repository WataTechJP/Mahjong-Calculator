...
const SCORE_PRESETS = {
  ...
  } as { child: Record<number, Record<number, number>>; dealer: Record<number, Record<number, number>> },
  tsumo: {
...
export function calculateScoreCost(params: {
  ...
  const scoreTable = isTsumo
    ? SCORE_PRESETS.tsumo
    : isWinnerDealer
      ? SCORE_PRESETS.ron.dealer
      : SCORE_PRESETS.ron.child;
  ...
  return { main: score as number, additional: 0 };
}
